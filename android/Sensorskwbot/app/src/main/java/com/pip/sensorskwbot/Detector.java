...
import com.pip.sensorskwbot.objectNav.ObjectNavFragment;
...
public abstract class Detector extends Network {
  ...
  protected Detector(Activity activity, Model model, Device device, int numThreads)
    ...
    parseTflite();
    pTimber.d("Created a Tensorflow Lite Detector.");
  }
  ...
  public List<Recognition> recognizeImage(final Bitmap bitmap, String className)
    ...
    pTimber.d("Timecost to convertBitmapToByteBuffer: %s", String.valueOf((endTime - startTime)));
    ...
    Trace.endSection();
    pTimber.d("Timecost to run model inference: %s", String.valueOf((endTime - startTime)));
    ...
    endTime = SystemClock.elapsedRealtime();
    pTimber.d("Timecost for postprocessing: %s", String.valueOf((endTime - startTime)));
